...
% introducing norms used to control the evolution of the coordinates $(I, K)$ and
...
% Also, we set $s = 0$ and $\sigma_0 = \sigma_x$ and deal only with the bulk flow;
% as discussed in Section~\REF, the observable flow can then be constructed as in
% \cite{ST-phi4} and the improved bounds with $s > 1$ are immediate.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Initial coordinates for the renormalization group}
\label{sec:K0bd}
...
% \end{equation}
% We continue to use the norms defined in Section~\ref{sec:Ncal} and Section~\ref{sec:Rpf1}.
We will need several properties of the $T_\varphi$ seminorm, whose proofs can be found in
...
% $\ggen_0$ must be chosen small depending on $L$.
...
regulator norms in \refeq{Gnormdef1}--\refeq{Gnormdef2},
...
% \end{equation}
% First, we extend the definitions of the regulators $\Gcal_0 = G_0, \tilde G_0^{\sf t}$,
...
\begin{equation}
\new{\|F_1F_2\|_{\Gcal_j} \le \|F_1\|_{\Gcal_j} \|F_2\|_{\Gcal_j}}
	% \quad
\text{ for }
	% \quad
...
Given a map \new{$K \in \Kspace_j$,
% with the property that $K(X) \in \Ncal(X^\Box)$ for all $X \in \Pcal_0$,
we define the $\Fcal_j(\Gcal)$} norms (for $\Gcal = G, \tilde G$) by
\new{
\begin{align}
\|K\|_{\Fcal_j(G)}        &= \sup_{X\in\Ccal_j} \ggen_j^{-f_j(a, X)} \|K(X)\|_{G_j} \\
\|K\|_{\Fcal_j(\tilde G)} &= \sup_{X\in\Ccal_j}
\ggen_j^{-f_j(a, X)} \|K(X)\|_{\tilde G_j^{\sf t}},
\end{align}}
with
\new{
\begin{equation}
    ...
    \end{cases}
\end{equation}
(recall that $|X|_j$ is the number of $j$-blocks in $X$).}
Here $a$ is a small constant;  its value is discussed below \cite[\eqref{step-e:T0dom}]{BS-rg-step}.
The \new{$\Wcal_j = \Wcal_j(\ggen_j, \volume)$} norm is then defined by
\new{
\begin{align}
\label{e:9Kcalnorm}
\|K\|_{\Wcal_j}
...
% THERE USED TO BE A MISTAKE HERE IN SAW-SA
\end{equation}
Moreover, $K^+_0$ is % left- and
...
to non-trivial $K_0$.
% and with the new domains $\domRG_j$ defined in terms of the weights
...
a finite-volume renormalization group flow $(V_j, K_j) \in \domRG_j$ for $0 \le j \le N$.
...
there is an infinite-volume renormalization group flow
\begin{equation}
...
of the renormalization group flow in $g_0$. However, once the flow has been
...
We wish to initialize the renormalization group with $(\nu_0, z_0)$ a solution
to the system of equations
\begin{align}
&\nu_0 = \nu_0^c(m^2, \newer{\gcc_0}, K^+_0(g_0, \gamma_0, \nu_0, z_0)), \label{e:mu0c}
\\
&z_0 = z_0^c(m^2, \newer{\gcc_0}, K^+_0(g_0, \gamma_0, \nu_0, z_0)) \label{e:z0c}
.
\end{align}
...
% $V_\pt$, which is independent of $\gamma_0$.
\end{proof}
