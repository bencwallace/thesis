...
$e^{-|\gamma_0|U_x^\pm}-1$ is roughly of size $|\gamma_0|\,|\varphi|^4 \approx |\gamma_0|/\ggen_0$.
...
\lbeq{reg0X}
    ...
    .
\end{equation}
It follows from the product property of the $T_\varphi$ norm that these norms obey the product property
\begin{equation}
...
write $F_X, F^\varphi_X : D \to \Ncal(X^\square)$ for the
maps defined by partial evaluation of $F$ at $X$ and at
$(X, \varphi)$, respectively. We say $F^\varphi_X$ is $C^k$
...
$F^{(i)} : D \to \Wcal^*_0(\ggen_0, \Zd)$ by $(F^{(i)}(t))^\varphi_X = (F^\varphi_X)^{(i)}(t)$ for $i = 1, 2$,
