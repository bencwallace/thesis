...
\end{cases}
\end{equation}
The susceptibility, finite-order correlation length, and critical point are defined
identically for any $n$ in terms of the corresponding two-point function. We recall
...
This will be discussed further in \REF.
...
By \REF, $\varphi$ is approximately constant on blocks of side $L^j$. In a sense,
...
\end{cases}
\end{equation}
When $n = 0$, it can be shown that the only marginal and relevant local monomials (up to constant
factors) that are Euclidean invariant and supersymmetric are given by
\begin{equation}
\tau_x,	\quad \tau_x^2, \quad \tau_{\Delta,x}, \quad \tau_{\nabla\nabla,x}.
\end{equation}
When $n \ge 1$, these are monomials are Euclidean invariant and $O(n)$-invariant
and there is an additional local monomial with these properties: the constant $1$.

We define $\Vcal$ to be the space \emph{local polynomials} (sums of local monomials)
$V_y$ of the form
\begin{align}
\lbeq{Vy}
V_y
...
\begin{equation}
\label{e:Vcalesig}
\Vcal(X) = \{V(X) : V \in \Vcal \},
\end{equation}
where $V(X)$ is defined as in \eqref{e:VX}.
We also make use of the subspaces $\Vcal^{(1)} \subseteq \Vcal$ consisting
...
The $\Vcal = \Vcal_j$ norm is equivalent to the $T_0(\ell_j)$ seminorm on $\Vcal(B)$
when $|B| = L^{jd}$:
\begin{equation}
\|V\|_\Vcal \asymp \|V(B)\|_{T_0(\ell_j)} = L^{jd} \|V_y\|_{T_0(\ell_j)}.
\end{equation}
...
\todo{Discuss the cumulant expansion.}
...
\end{equation}
where $W = W(V)$ is an explicit polynomial that is quadratic in $V$; see
...
Following an approximate change of coordinates with $O(V^3)$ errors, the perturbative
...
\end{equation}
We track $Z_j$ using \emph{renormalisation group coordinates} $\zeta_j \in \R$,
...
\label{e:IcircKnew}
	...
	.
\end{equation}
The coordinate $I_j = I_j(U, \cdot)$ is defined by
\begin{equation}
I_j(U, X)
	=
\prod_{B \in \Bcal_j(X)} e^{-U(B)} (1 + W_j(B, U)), \quad X \in \Pcal_j,
	\quad
U \in \Vcal^{(0)}.
\end{equation}
...
\commentbw{Define symmetries and determine what are the right symmetries for $n > 1$.}
...
\commentbw{Consider using $V$ for bulk and $U$ for bulk plus observables. In saw-sa,
$V^+$ is used extensively for bulk. Need to check that $U$ is not too important in clp.}
...
\begin{equation}
\label{e:IK0def}
I_0^+(X) = I_0(V^+_0, X) = \prod_{x\in X} e^{-V^+_{0,x}},
	\qquad
...
consider maps $V_+ = V_{j+1}$ and $K_+ = K_{j+1}$ that act on the domain
$\domRG_j$ and map into $\Vcal_{j+1}^{(1)}$, $\Kcal_{j+1}$, respectively.
