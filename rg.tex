...
group map goes back to \cite{BY90}. Although the notion of a covariance
decomposition was implicit in Wilson's work, its utility in the context of the
renormalisation group was probably first articulated in \cite{BCGNOPS78}.
\emph{Finite-range} decompositions were first used in this context in
\cite{MS00} based on a suggestion of Brydges. \emph{Lattice} covariance
decompositions were constructed in \cite{BGM04} and used in \cite{Bryd09} to
implement the renormalisation group on the Euclidean lattice as well as a
\emph{hierarchical} lattice, which was introduced by Dyson \cite{Dyso69}. A
version of the weakly self-avoiding walk on the hierarchical lattice was
defined and studied by a renormalisation group method in
\cite{BEI92,BI03c,BI03d}.
