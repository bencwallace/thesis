...
Formally, we let $\Ncal'$ denote the extension of $\Ncal^\varnothing$
...
decomposition
\begin{equation}
\Ncal = \Ncal^\varnothing \oplus \Ncal^a \oplus \Ncal^b \oplus \Ncal^{ab},
\end{equation}
where $\Ncal^a$ consists of elements of the form $\sigma_a F$ with $F\in\Ncal^\varnothing$
...
\begin{equation}
\label{e:obs-decomp}
F = \new{F_\varnothing + \sigma_0 F_0 + \sigma_x F_x + \sigma_0 \sigma_x F_{0x}},
	\quad
...
\item
Symmetry: (i) \new{$\pi_\varnothing K$ is Euclidean-covariant;
(ii) if $n = 0$, then $K$ is gauge-invariant and $\pi_\varnothing K$ is supersymmetric
and has no constant part; if $n \ge 1$, then $\pi_\varnothing K$ is $O(n)$-invariant.}
% and (iii) $\pi_\varnothing K$ is supersymmetric and has no constant part if $n = 0$
...
\commentbw{Sec 2.8: you make reference to \cite{BSW-saw-sa} but this is for $n=0$ yet you seem to
be claiming things here for all $n \ge 0$?
