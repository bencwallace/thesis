...
We initialize the renormalisation group with the critical parameters of
Theorem~\ref{thm:rhatflow}. Then for any $N$, the representation
\eqref{e:IcircKnew} at scale $j = N$ becomes
\begin{equation}
\label{e:ZNINKN}
Z_N = e^{\zeta_N} (I_N(\Lambda) + K_N(\Lambda))
\end{equation}
with $(V_N, K_N) \in \domRG_N$; in particular, using
\eqref{e:Vnormdef}, \eqref{e:Wbilinbd}, and \eqref{e:T0dom},
\begin{align}
\label{e:unu-bd}
u_N |\Lambda_N| = O(1),
	&\qquad
...
\end{align}
Now \eqref{e:ZNINKN} and \eqref{e:chibarm} together with the definitions of $I_N$ and $V_N$
imply that
\begin{equation}
...
\todo{The following need to be simplified to $\gamma_0 \ge 0$.}
...
By \eqref{e:WKbd} and Lemma~\ref{lem:deriv-norm-bds}, the last two terms of
\refeq{GK} vanish as $N \to \infty$, and
\begin{equation}
\frac{1}{1+z^c_0} G_x(g, \nu; n) = \frac{1}{2} (q_{0,\infty} + q_{x,\infty}),
\end{equation}
where the $q_{u,\infty}$ are evaluated at the renormalized mass $m^2$.
...
\quad u = 0, x,
\end{equation}
where $R^{q_u}_i$ is the coefficient of $\1_{y=u}\sigma_0\sigma_x$ (recall \refeq{Vy})
in $R_{+,i}$.
...
\todo{We also use $\ggen_j^{-2} = O(\gbar_j^{-2})$, $\chicCov_i \le 1$, $\ell_0^2 \le O(1)$,
...
For the term $O(\gbar_{j_x}) G_x(0, m^2)$ in \refeq{rx},
...
\begin{equation}
   ...
   .
\end{equation}
By \eqref{e:rab-bound-scales-bis} and \refeq{Phi-def-jc},
