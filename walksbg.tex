...
In this appendix, we prove Proposition~\ref{prop:finvol}. Since we are only
dealing with walks, we can drop the parameter $n = 0$ from
our notation. We will use $n$ to denote a different quantity below.
...
Let $P^n$ be the projection of $\Zd$ onto the discrete torus of side $n$,
which we denote $\Z_n^d$. Then $P^n$ has a natural action
on the path space $(\Zd)^{[0,\infty)}$. We let $X^n = P^n(X)$ be the projection
of $X$ and note that $X^n$ is a simple random walk on $\Z^d_n$.
...
The following is a re-statement of Proposition~\ref{prop:finvol}.
...
\begin{proof}
It suffices to prove \eqref{e:Givlc-re} and \eqref{e:chilim-re}. Analyticity is
a property of the Laplace transform and uniform convergence on compact sets follows
from Montel's theorem. For pointwise convergence, we will only prove the case $\gamma \ge 0$.
The proof for $\gamma < 0$ can be found in
\cite{BSW-saw-sa}.
