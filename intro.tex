...
\commentbw{Define $\asymp$ and maybe mention $\approx$}
...
(\todo{\REF, see Madras-Slade}) that $g(z) \sim (r - z)^{-\alpha-1}$ as
...
\commentbw{Incomplete}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{The Laplace transform}

The \emph{Laplace transform} of a function $f(T)$ for $T \in \R$ is
\begin{equation}
\label{e:laplace-transform-def}
G(\nu) = \int f(T) e^{-\nu T} \; dT.
\end{equation}
Once again, there is a relationship between the asymptotics of $f(T)$ as $T\to\infty$
...
We will also assume that $\graph$ is \emph{(vertex-)transitive} meaning that for all pairs
of distinct
...
\commentbw{Translation invariant case: $C(x) = C(0, x)$}
...
\commentbw{Maybe remove the following.}
...
\end{equation}
where $e \in \Zd$ is a unit vector whose choice is irrelevant by \todo{Euclidean invariance}
of this model.
...
are available for their critical exponents. In fact, even basic facts regarding the
qualitative behaviour of many spin systems is unknown.
