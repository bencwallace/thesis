...
$(\tilde X_n)_{n\in\Z_+}$, called the \emph{random walk} with transition matrix $P$.
In particular, we can let $P_{xy} = -Q_{xy} / Q_{xx}$ if $x \ne y$ and $P_{xx} = 0$.
...
Suppose that $M$ is a positive-definite symmetric matrix.
Then the Gibbs measure induced by the Hamiltonian
\begin{equation}
H(\varphi) = \frac{1}{2} \varphi \cdot M \varphi
\end{equation}
is a Gaussian measure, known as the discrete Gaussian field with covariance $C = M^{-1}$.
...
\left(
  ...
  \frac{1}{2} \nu |\varphi_x|^2
    +
  \frac{1}{2} \varphi_x \cdot (M \varphi)_x
\right).
\end{equation}
Due to the presence of the quartic term, $M$ can be any symmetric
matrix\footnote{Positive-definiteness is required for the Gaussian measure to be
integrable. There is no loss of generality in assuming symmetry of $M$.}
An important special case corresponds to the choice $M = -\Delta$.

\todo{Discuss ferromagnetism}


\subsubsection{The $O(n)$ spin model}

\todo{This is for $M = -\Delta$. Do general $M$ by extracting the diagonal part.}

If we take the limit $g\to\infty$ with $\nu = -g / 2$, the $|\varphi|^4$ spin model converges
