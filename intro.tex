...
Let $\Omega = U^n \times \R^{3n}$, where $U \subset \R^3$. Let us denote an element of
...
This situation can be modeled by considering a system, called system $A$, in contact
...
Given any state space $\Omega$ (not necessarily of point particles) and Hamiltonian
$H : \Omega \to \R$, we define the \emph{Gibbs measure} at inverse temperature $\beta > 0$
on $\Omega$ to be the probability measure of the form
\begin{equation}
\frac{1}{Z} e^{-\beta H(\omega)} d\omega_1.
\end{equation}
...
By considering a physical system which is free to exchange both energy and particles with its
environment, a similar derivation as above leads us to the definition of the \emph{grand
canonical ensemble} with Hamiltonian $H$, given by
\begin{equation}
\frac{1}{Z_{\beta,\nu}} e^{-\beta (H(\omega) - \nu |\omega|)},
\end{equation}
where $|\omega|$ is a number representing the number of particles in the statem $\omega$.
For mathematical purposes, the $\nu |\omega|$ term can be absorbed into the Hamiltonian.
Thus, the grand canonical ensemble is itself a Gibbs measure and we will only loosely
distinguish between the canonical and grand canonical ensembles. 


\section{Lattice models}

Let $\graph = (\vertices, \edges)$ be a graph and let $A$ be a symmetric
$\vertices\times\vertices$ matrix.


\subsection{Models of walks}

Let $\Wcal_T$ denote the set of
right-continuous paths $[0, T] \to \vertices$.
...
Suppose we are given such a model $d\mu_T$. The associated
canonical partition function associated with $d\mu_T$ is given by
\begin{equation}
c_T = \mu_T(1).
\end{equation}
There is also a natural measure on $\Wcal$ defined as follows:
given a parameter $\nu \in \R$ (called the \emph{killing rate} or \emph{fugacity}),
\begin{equation}
\mu(f)
  =
\frac{1}{\chi(\nu)} \int_0^\infty dT \; e^{-\nu T} \int_{\Wcal_T} d\mu_T(\omega) f(\omega),
\end{equation}
where the grand canonical partition function
\begin{equation}
\chi(\nu) = \int_0^\infty dT \; e^{-\nu T} c_T
\end{equation}
is known as the \emph{susceptibility}.

Suppose we restrict our attention to walks $\omega \in \Omega_T$ with $\omega_T = x$.
We define the \emph{two-point function} by
\begin{equation}
G_x(\mu) = \int dT \; e^{-\nu T} c_T(x),
\end{equation}
where
...
The sequence $\tau_n$ of jump times is defined inductively by setting $\tau_0 = 0$ and
\begin{equation}
\tau_{n+1} = \inf(t > \tau_n : X_t \ne X_{\tau_n}).
\end{equation}
Suppose that the sequence $\mu_T$ is chosen so that $\tau_n = n$ for all $n$. Then $\mu_T$
is supported on a finite set that can be identified with $\mu_{\lfloor T \rfloor}$.
If we assume further (using this identification) that $\mu_T(\omega) = \mu_{\lfloor T \rfloor}(\omega)$
for all $\omega \in \Wcal_T \simeq \Wcal_{\lfloor T \rfloor}$, then $\mu_T$ is said to model
\emph{discrete-time} walks.
In this case, it suffices to study the sequence $\mu_n$ for $n \in \Z_+$ (from which the
$\mu_T$ can be recovered). Moreover, we can write
\begin{align}
\chi(\nu)   &= \frac{1 - e^{-\nu}}{\nu} \sum_{n=0}^\infty c_n z^n, \quad z = e^{-\nu}.
\end{align}

Many models of walks take the form
\begin{equation}
d\mu_T(\omega) = \frac{1}{Z_T} e^{-H_T(\omega)} d\lambda_T(\omega),
\end{equation}
where $d\lambda_T$ is a probability measure on $\Wcal_T$ and the function $H_T : \Wcal_T \to \R$
is known as the \emph{Hamiltonian}.
...
Let $Q$ denote a $\vertices \times \vertices$ matrix with $Q_{xy} = 0$ whenever
$\{ x, y \} \notin \edges$.
Let $X = (X_T)_{T\ge 0}$ be the $V$-valued
Markov process with generator $Q$ and let
$A = e^Q$ so that $(X_n)_{n\in\Z_+}$ is a
$V$-valued Markov chain with transition matrix $A$.
For $v \in \vertices$, let $E_v$ denote the expectation
with respect to the process $X$ conditioned so that $X_0 = v$.
...
\end{equation}
We also let a $V \times V$ matrix $A$ act on $\varphi$ via
\begin{equation}
(A\varphi)^i_x = \sum_{y\in\vertices} A_{xy} \varphi^i_x.
\end{equation}
...
Suppose that $A$ is positive-definite. Then the Gibbs measure induced by the Hamiltonian
\begin{equation}
H(\varphi) = \frac{1}{2} \varphi \cdot A \varphi
\end{equation}
is a Gaussian measure, known as the Gaussian field with covariance $C = A^{-1}$.
In particular, when $C = (-\Delta + m^2)^{-1}$ with $m^2 \ge 0$, we get the \emph{discrete Gaussian
free field} (DGFF). The DGFF is said to be \emph{massive} with \emph{mass} $m^2$ when $m^2 > 0$,
and is said to be \emph{massless} when $m^2 = 0$.
...
To do: replace $\Delta$ by $A$.


\subsubsection{The $O(n)$ spin model}

If we take the limit $g\to\infty$ and $\nu\to-\infty$ in the appropriate way, we get the
$O(n)$ spin model with $S = S^{n-1}$ the $(n-1)$-sphere in $\R^n$ and with Hamiltonian
\begin{equation}
H(\varphi) = -\frac{1}{2} \varphi \cdot A \varphi.
\end{equation}
...
\todo{Diagonal part doesn't matter? Ferromagnetic $A$}
...
Specialize to $\Zd$. Specialize to infinite-volume limits along tori (mainly
to avoid pathological sequences of finite-volume approximations).}
