...
$\Omega$. For this reason, we temporarily restrict our attention to finite graphs:
\begin{equation}
|\vertices| < \infty.
\end{equation}
In many cases, the Hamiltonian
will depend on one or more parameters; in this case, adjusting $\beta$ is equivalent
to rescaling these parameters and so we will usually drop $\beta$
(or set $\beta = 1$).
...
\todo{Restrict this discussion to spin systems}
...
\todo{Don't work in so much generality. Just mention SRW and SAW first. Then
make a section ``Gibbs measures on walks'' or the like and discuss the other
models, just in continuous time.}
