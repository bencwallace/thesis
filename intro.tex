...
group}. This idea, which was enormously successful, led to his Nobel prize and has
...
to those for generating functions \cite{Widd41}. For instance,
$f(T) \sim A T^\alpha$ \todo{implies that} $G(\nu) \sim A' (\nu - \nu_0)^{-(\alpha+1)}$
as $\nu\downarrow\nu_0$. The converse holds when $f$ is monotonic.
...
of the $n$ particles. The Hamiltonian is typically smooth and the dynamics of such a system
...
\commentbw{New approach: increasing complexity. Consider Hamiltonians that are
polynomials of even degree etc. Get $O(n)$ model at the end by limit.}

The Ising model on $\Zd$ was introduced by Willhelm Lenz \cite{Lenz42}. Lenz
suggested it as a problem for his student Ernst Ising, who determined in 1924
\cite{Ising25} that there is no phase transition when $d = 1$. In 1936, Rudolf
Peierls \cite{Peierls36} demonstrated that a phase transition occurs for $d > 1$
and in 1944, Onsager \cite{Onsager44} provided an exact calculation of the free
energy in $d = 2$ with $h = 0$.

\commentbw{Heisenberg model; general spin systems; subtlety of $n = 2$}

For more information on spin systems, we refer to \cite{Georgii11} and \todo{Friedli-Velenik}.

\commentbw{Maybe remove the following. Either that or talk about something interesting,
...
\end{equation}
\emph{Wick's theorem} gives an expression for the correlations:
...
\begin{example}[The Gaussian free field]
\commentbw{Maybe give this example in section on spin systems}
...
\commentbw{Following needs work}
...
\commentbw{Aizenmann, Fr\"{o}hlich, FMRS, etc.}
...
\commentbw{Do something about the following paragraph}
...
\commentbw{Motivate historically using SAW. Then just do WSAW-SA with WSAW as
an easier version of SAW.}
