...
% \commentbw{Explain why this is an ``irrelevant perturbation'' of the usual
% model.}
...
\xi_p(g, \gamma, \nu; n)
	=
\left(\frac{\sum_{x\in\Zd}
|x|^p G_x(g, \gamma, \nu; n)}{\chi(g, \gamma, \nu; n)}\right)^{1/p}.
\end{equation}
\newer{An analysis of $\xi_p$ requires less control over the behaviour of the
two-point function than would be needed to study the correlation length $\xi$.
Our main result includes a statement regarding the critical behaviour of
$\xi_p$ in $d = 4$.}
% \commentbw{Explain why we introduce $\xi_p$: easier to control/interpretation
% for walks.}
...
$D R(f_*)$. A computation shows that this map has \newer{eigenfunctions $H_i$} given by Hermite polynomials with eigenvalues $\lambda_i = 2^{1-i/2}$ for integers $i \ge 0$.
\newer{The relevant directions (in the sense of Wilson's renormalization group) are those
in which the linearized map is expanding; thus, the relevant subspace is spanned in
this case by $H_0$ and $H_1$. Similarly, the irrelevant directions are spanned by $H_i$
with $i > 2$ while $H_2$ spans the marginal subspace.}
...
\newer{measure $\mu_n$} for each $n$. This gives us a simple model of a linear polymer in a good
...
\nu_c = \nu_c(\gcc, \gamma) = \inf\{ \nu : \chi(\gcc,\gamma,\nu) < \infty \},
\end{equation}
we expect the susceptibility to diverge. \newer{The susceptibility is a partition
function for walks with $\nu$ playing the role of an external field, so this
divergence would be indicative of a phase transition as discussed in
Section~\ref{sec:inf-vol}. In a certain sense \cite{DKY14,GI95}, paths in the
$\nu > \nu_c$ phase
should scale as geodesics while paths for $\nu < \nu_c$ should be space-filling.}
...
($\nu < \nu_c$ for WSAW-SA).
However, the authors of \cite{DKY14} considered SAW on a discretized bounded
...
\begin{rk}
\label{rk:bubble}
Define the free \emph{bubble diagram}\footnote{\newer{This is sometimes represented by
a graph consisting of two edges (forming a ``bubble'') joining a vertex labeled $0$
(denoting the origin) to an unlabeled vertex (denoting an arbitrary point $x$ that
is summed over $\Zd$).}}
to be the $\ell^2(\Zd)$ norm of the massive
...
group method in \cite{BEI92,BI03c,BI03d,GI95}.}
...
Let $d \ge 2$.
\newer{The predicted behaviour of self-avoiding walk with attraction is discussed in
\cite{Vand98,Holl09}. The predicted phase diagram and critical exponents
are shown in
Figure~\ref{fig:theta-curve}. Generally speaking, one might expect the self-attraction
to dominate when $\gamma > g$ so that the walk typically remains in a bounded region,
i.e.\ $\nubar = 0$. A proof of this fact appears in \cite{HK01a} for a related model in
$d = 1$. The authors of \cite{HK01a} also conjecture that $\nubar = 1/(d+1)$ for
$g = \gamma$ and this is established for $d = 1$ in \cite{HKK02}.}

\newer{While it is natural to expect the self-avoidance to dominate when $\gamma < g$,
a \emph{collapse transition} is believed to occur as $\gamma$ crosses the
$\theta$-curve $g \mapsto \gamma_\theta(g)$.
% For $\gamma$ above the $\theta$-curve but below the line $\gamma = g$,  $\nubar = 1/d$, which reflects the fact that a typical walk of length $T$ curls into a
% ball of volume approximately $T$.
The value of the critical exponent $\nubar$ for $\gamma = \gamma_\theta$ is predicted to
be given by $\nubar_\theta = 4/7$ if $d = 2$ and $\nubar_\theta = 1/2$ if
$d \ge 3$ (with a logarithmic correction in $d = 3$). However, very little is known
rigorously about the $\theta$-curve. Only recently has a collapse transition been shown
to exist for a model of \emph{prudent} self-avoiding walk with self-attraction \cite{PT16}.

% The predicted values for $\nubar$
% in the other regions of the phase diagram are shown in the figure.
% In particular, for $\gamma > g$, the WSAW-SA is expected to undergo a collapse transition
% ($\nubar = 0$).

\begin{rk}
De Gennes \cite{Genn75} related the behaviour of polymers in poor solvents to the
tricritical behaviour of certain spin systems. Such systems typically possess two
phase transitions: one corresponding to each critical point on a line of critical
points and one corresponding to a \emph{tricritical} point given by an endpoint of
this line.
For the WSAW-SA, the tricritical point should be given by $(g, \gamma_\theta, \nu_c)$.
Moreover, the upper-critical dimension for such tricritical behaviour is $d = 3$,
consistent with the predicted values of $\nubar_\theta$.
\end{rk}}

% \commentbw{$\gamma > g$ compare to pure self-attraction (Brydges-Slade, Bolthausen-Schmock, Ooono). $\gamma=g$ conjecture by van der Hofstad-Klenke.
% $\gamma < g$ walk in ball. Theta-curve/two transitions due to tricritical
% behaviour.}
