...
and $d_x = 2d$.
\commentbw{This is really two examples as such.}
\end{example}
...
in the Ising model if $d > 2$. Peierl's argument was made rigorous in \cite{Griffiths64},
but even before that Onsager \cite{Onsager44} provided an exact computation of the
free free energy in $d = 2$, which uncontrovertibly demonstrated the existence of a
...
Let $B_N = [-N, N]^d \cap \Zd$ and let $\partial V = \partial B_N$ be the external
vertex boundary of $B_N$. For each $x\in\partial\vertices$, let $b_x \in \{-1, 0, 1\}$
be a choice of \emph{boundary condition}
% let $h \in \R$ be an \emph{external field},
...
\commentbw{Get rid of boundary conditions}
...
\commentbw{Define discrete gradients}
...
\commentbw{Mention that this is the primary case of interest in QFT.}
