...
H_{g,\nu}(\varphi)
  =
\sum_{x\in\vertices} U_{g,\nu}(\varphi_x) - \frac{1}{2} \varphi \cdot \jay \varphi,
\end{equation}
where the \emph{single-spin potential} $U_{g,\nu}$ is defined by
\begin{equation}
U_{g,\nu}(t)
	=
\frac{1}{4} g |t|^4
...
Gibbs measure for the $O(n)$ model in the limit $g\to\infty$
with $\nu = -(d_0 + g / 2)$.
\end{example}
...
\todo{Include this for consistency with $|\varphi|^4$ model}
...
Our main results concern the two-point function, susceptibility, and finite-order
correlation length of both models. For the $|\varphi|^4$ model, we take the infinite-volume
limit using periodic boundary conditions. That is, for $n \ge 1$,
\begin{equation}
