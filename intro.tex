...
More generally, the Feynman-Kac formula expresses solutions to \eqref{e:linear-pde}
as averages with respect to a Brownian motion.
\end{rk}
...
the corresponding Gibbs $\mu^{(N)}_{\beta,h}$ measure and let $Z^{(N)}_{\beta,h}$
be the partition function. The \emph{free energy} is defined in finite volume by
\begin{equation}
F^{(N)}_{\beta,h}
	=
...
of the two-point function, which will be discussed in more detail in Section~\ref{sec:crit}.
...
In this thesis, we are mainly interested in the approach to the critical point from
the single-phase region: $\beta\uparrow\beta_c$. Thus, we will avoid the issue of
