...
\todo{This (or above) might be a good place to define the exact quantities we will study,
i.e.\ as limits of finite-volume quantities on $\Lambda_N$}
...
reasonable to expect rapid (i.e.\ summable) decay of $G_x(g, \nu; n)$ in $|x|$ for
...
\xi_p(g, \nu; n)
	=
\left(\frac{\sum_{x\in\Zd} |x|^p G_x(g, \nu; n)}{\chi(g, \nu; n)}\right)^{1/p}.
\end{equation}
...
$\xi_p$ should come from $|x| \le \xi = \xi(g, \nu; n)$. For such $|x|$,
$G_x(g, \nu; n) \approx C |x|^{-\alpha}$ and so
\begin{equation}
\sum_{x\in\Zd} |x|^p G_x(g, \nu; n)
	\approx
...
For simplicity, let us drop $g$ and $n$ from the notation.
...
depend on $n$ and $d$ and independent of $g$ and when $g > 0$. In fact,
...
\todo{Do this in such a way as to be consistent with the definitions used in the
main results.}
...
\subsubsection{The \texorpdfstring{$|\varphi|^4$}{phi4} model}
...
\begin{equation}
G_x(\gcc, \gamma, \nu; n) = \lim_{N\to\infty} G_{x,N}(\gcc, \gamma, \nu; n),
\end{equation}
where
\begin{equation}
G_{x,N}(\gcc, \gamma, \nu; n)
	=
\langle \varphi_0 \cdot \varphi_x \rangle_{\gcc,\gamma,\nu,N}.
\end{equation}
...
U_{\gcc,\gamma}(f)
	=
\gcc \sum_{x\in\Zd} f_x^2 - \frac{1}{2 d} \gamma \sum_{x\in\Zd} \sum_{y \sim x} f_x f_y
\end{equation}
and let
\begin{equation}
U_{\gcc,\gamma,T} = U_{\gcc,\gamma}(\lt_T) = \gcc I_T - \frac{1}{2 d} \gamma C_T.
\end{equation}
Let
\begin{equation}
c_T = E_0 \big(e^{-U_{\gcc,\gamma,T}}\big),
	\quad
c_T(x) = E_0 \big(e^{-U_{\gcc,\gamma,T}} \1_{X_T=x}\big).
\end{equation}
The WSAW-SA two-point function and susceptibility are defined by
\begin{equation}
G_x(\gcc, \gamma, \nu; 0)
	=
\int_0^\infty c_T(x) e^{-\nu T} \; dT
...
\chi(\gcc, \gamma, \nu; 0)
	=
\int_0^\infty c_T e^{-\nu T} \; dT = \sum_{x\in\Zd} G_x(\gcc,\gamma,\nu;0).
\end{equation}
...
\subsection{The \texorpdfstring{$n\to0$}{n to 0} limit}
