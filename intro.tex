...
i.e.\ $\nubar = 0$)
and the self-avoidance to dominate when $\gamma < g$ (so that the walk behaves like
the WSAW). A proof of the former statement appears in \cite{HK01a} for a related model in
$d = 1$. The authors of \cite{HK01a} also conjecture that $\nubar = 1/(d+1)$ for
$g = \gamma$ and this is established for $d = 1$ in \cite{HKK02}.}

\newer{However, a \emph{collapse transition} is believed to occur as $\gamma$ crosses the
...
this line. Moreover, the upper-critical dimension for such a system is $d = 3$.
