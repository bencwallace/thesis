...
The following shows why this is the case. Also discuss the large field
problem here.}

For this discussion, let us set $\sigma_0 = \sigma_x = 0$ and $n = 0$.
Thus, we can drop the coupling constants $\lambda, q, u$ from the notation.
In this setting, our goal, given $(V, K)$, is to construct $(V_+, K_+)$
such that, with $I = I(V)$ and $I_+ = I(V_+)$,
\begin{equation}
\label{e:IcircKsimp}
...
$(V, K) \mapsto (V_+, K_+)$ can be iterated an arbitrary number of times as in
...
\subsection{Simplified renormalisation group step}

A possible definition of the map $(V, K) \mapsto V_+$ is suggested by perturbation
...
by block-factorization. Then \todo{by associativity of $\circ$ and the binomial
formula},
\begin{align}
(I \circ K)(\Lambda)
...
\commentbw{First look at bound on $h$. Easy to see that $\delta I$ is
$O(\delta V)$ (discuss cumulant expansion earlier on). Bounding $\delta V$
requires bound on covariance (straightforward computation). This motivates
the covariance bound needed and proved in the next section. Not surprisingly,
the bound on the covariance comes up all over the place.}

\commentbw{Next discuss $k$.
Even if $K = 0$, $K_+$ will contain a $\delta I$, so extraction is
needed. This is done using $\Loc$ and the crucial contraction.}

\commentbw{There are also stability bounds, which we require bound on regulator;
this is discussed in detail in rg-IE. I should probably at least briefly explain
this here since the change of regulator norm is such a fundamental part of this
method.}

Our main task then is bound $h$ and $k$. However, we immediately encounter the
following problem. Suppose that for some block $B\in\Bcal$,
\begin{equation}
K(B) = g^3 \sum_{x\in B} \varphi_x^6.
\end{equation}
Our heuristics suggest that $\varphi_x^6$ is ``irrelevant'', so it's potential
appearance in $K$ may not come as a great surprise. But the integration of such
$K(B)$ will contain a term
\begin{equation}
\binom{6}{2} g^3 \sum_{x\in B} \varphi_x^4 C_{00},
\end{equation}
which is a marginal term.
...
\commentbw{Motivating example: for any $\varphi$, there is a function that is
not integrable with respect to the Gaussian measure but whose $T_\varphi$
norm vanishes.}
...
Theorem~\ref{thm:step-mr-fv}
holds,
thereby completing the proof of Proposition~\ref{prop:R}.
The key steps in
...
while the third is \emph{not} when $s >0$ and $j_m < j_x$.
...
The stability domain $\DV_j$ is defined in \cite[\eqref{IE-e:DV1-bis}]{BS-rg-IE}.
We modify $\DV_j$ only for the coupling constant $q$,
by replacing $r_q$ in
\cite[\eqref{IE-e:h-coupling-def-1-bis}]{BS-rg-IE} by
\begin{equation}
\lbeq{newrq}
