\chapter{The renormalization group step}
\label{sec:RGstep}
...
% \commentbw{Ch 4 line 1:  again you have $s>1$, is it $s>0$?}
...
\new{Then by \eqref{e:binom} and associativity of the circle product},
\begin{align}
(I \circ K)(\Lambda)
...
% \commentbw{3 lines above (4.1.10):  $\delta I$ is like $\delta V$, not $(\delta V)^3$.}

By the definition \eqref{e:Idef} of $I$, we expect that \new{$\delta I = O(\delta\Vp)$}
...
\new{Covariance terms such as those above
% occur at higher-order and
can be estimated using Proposition~\ref{prop:Cdecomp}.}
% \commentbw{(4.1.11): don't understand what you mean by sentence following this eqn.
% What is the ``higher order'' referring to?}
...
Thus, the true definition of the renormalization group map constructed in
...
\newer{The map $V_+$ is a perturbation of $\Vpt$ whose size is determined by the
norm of $K$. In particular, for $(V, K) \in \domRG$ we get the first bound of
\eqref{e:RKplus} (recall \eqref{e:Rplusdef}), which ensures that the flow
of coupling constants exhibits the same qualitative behaviour as the perturbative
flow.

On the other hand, if $K$ is large, then the resulting perturbation may have a
non-trivial effect on the flow of coupling constants, resulting in different critical
behaviour than that predicted by the perturbative flow.}

% \commentbw{Rk 4.1.2: if you don't want to say what $V_+$ is then I think you need to
...
\new{Recall that the sequence $\chicCov_j$ was defined in \eqref{e:chicCov-def}.}
...
% \commentbw{Lemma 4.2.1: I want to say recall the defn of $\vartheta$ but I think I
% already noted that you don't give that defn.  I think you need it and
% need to recall it here.}
...
% The estimate \cite[\eqref{IE-e:CLbd}]{BS-rg-IE} is wasteful in that it does not make
% any use of the factor $(1+m^2L^{2(j-1)})^{-k}$ in \refeq{Phinorm2} beyond extraction
% of the factor $\chicCov_j$.
...
\begin{equation}
    ...
    .
\end{equation}
\new{By definition of $\chicCov_j$,
% (see \cite[Section~\ref{IE-sec:frp}]{BS-rg-IE}),
...
\end{equation}
\end{lemma}
% \commentbw{Lemma 4.2.2: OK here you do indeed want $s > 1$.}
\begin{proof}
...
% To do so, we make use of the renormalization group flow $(V_j,K_j)$
...
% The estimates on the renormalization group flow and remainder terms provided in those papers
...
% \commentbw{p. 73 end of 1st para on Loc:  Parenthetic sentence should start with capital letter.}
...
\new{We define
\begin{equation}
\tilde I_\pt(B) = e^{-\Vpt(B)} (1 + W_{j+1}(\Vpt, B)),
	\quad
B\in\Bcal_j
\end{equation}
and extend this to $\tilde I_\pt(X) = \tilde I_\pt^X$ for $X\in\Pcal_{j+1}$ by block-factorization.}

% \commentbw{Above Prop 4.3.3: you could define $\tilde{I}_{\pt}$ here, it is not complicated.}
...
% of order $p$.  The renormalization group analysis is predicated on fixed (but arbitrary)
