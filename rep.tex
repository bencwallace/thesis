...
\sum_{|e|=1} (\nabla^e \tau)_x^2.
\end{align}
\new{The forms $\tau_x$ are special due to the following remarkable property
of the super-expectation (see \cite{BIS09}):
\begin{equation}
\label{e:tau-iso}
\int e^{-S_A} F(\tau) = F(0).
\end{equation}
Note that \eqref{e:self-normal} is an immediate consequence of this fact.}
...
of norm and we provide a detailed verification that the estimates on the renormalisation
...
the parameters $g_0, \nu_0, z_0$ and a version of the implicit function
theorem is consequently required for the identification of critical parameters
such that the renormalisation group can be initialized on its stable manifold when
...
we restrict our attention to the more interesting case of $\gamma_0 \ge 0$ for
simplicity.

Prior to proving Theorems~\ref{thm:step-mr-fv} and \ref{thm:rhatflow}, we show in
...
$\gamma = 0$ first appeared in \cite{BSTW-clp}.
