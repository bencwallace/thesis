...
For the $O(n)$ spin model with interaction $J_{xy} = \1_{x\sim y}$, an uncontrolled
...
The super-expectation possesses the remarkable property
\begin{equation}
\label{e:tau-iso}
\int e^{-S_A} F(\tau) = F(0).
\end{equation}
In particular, the super-expectation is self-normalizing:
\begin{equation}
\label{e:self-normal}
\Ex_C 1 = \int e^{-S_A} = 1.
\end{equation}
