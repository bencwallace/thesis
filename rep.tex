...
In the other direction, one can consider studying walks by looking for corresponding
spin systems. In \cite{Genn72}, De Gennes argued that the self-avoiding walk
corresponds to an $n \to 0$ ``limit'' of the $O(n)$ spin model. Since $n$ is
...
Parisi and Sourlas \cite{PS80} and, independently, McKane \cite{McKa80}, discovered
...
An excellent reference on the connections between random walks and spin systems
as well as their applications can be found in \cite{FFS92}.
...
\commentbw{Non-zero contributions vanish for loops when $n = 0$}
...
In this section we describe an integral representation of the of WSAW-SA, which is a special
case of a result of Brydges, Imbrie, and Slade \cite{BIS09}. We restrict out attention to the
discrete torus $\Lambda$.
